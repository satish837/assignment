...
export default function TabbedSection() {
  ...
  <h2 className="text-4xl font-medium bg-gradient-to-r from-white via-gray-300 to-white text-transparent bg-clip-text lg:text-5xl lg:max-w-3xl mx-auto">
    <span className="block mb-4">Powering Payments Across </span> <span className="block">Industries</span>
  </h2>
</div>


        <div className="w-full rounded-2xl p-2 py-4 bg-[url('/tab-bg.jpg')] bg-cover bg-right bg-no-repeat">
        
         {/* Tab Buttons */}
         <div className="w-[80%] mx-auto flex justify-center gap-4 mb-5 border-2 border-[#182834] rounded-xl overflow-hidden p-1">
  {tabs.map((tab) => (
    <button
      key={tab.id}
      onClick={() => setActiveTab(tab.id)}
      className={`w-full px-5 py-3 rounded-xl text-2xl font-medium transition duration-200 ${
        activeTab === tab.id
...
/>
        ...
        {/* Content Area */}
        <div className="w-full px-8 md:px-12 md:py-0 flex flex-col md:flex-row items-center justify-between gap-10">
          {/* Left Content */}
          <div className="w-full md:w-1/2">
            <h3 className="text-4xl font-normal mt-6 mb-6 w-2xl">{current?.heading}</h3>
            <p className="text-gray-400 text-lg mb-6">{current?.description}</p>

            <ul className="space-y-4 text-sm">
              {current?.points.map((point, idx) => (
                <li key={idx} className="flex items-center gap-6">
                  <span className="text-lg"><img
                src={point.icon}
                alt={current?.label}
          ...
          <div className="w-full md:w-1/2 flex justify-center">
            <div className="relative w-[280px] h-[280px] md:w-full md:h-auto">
              <img
                src={current?.image}
                alt={current?.label}
                className="w-full h-full object-contain transition-all duration-300"
